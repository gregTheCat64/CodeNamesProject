...
fun getCards(isRedTurn: Boolean):Array<Card?>{
    ...
    for (w in 0.. wordList.size){
        ...
        when (randomColor) {
            Color.BLACK -> if (blackCard<maxBlackCard) blackCard++ else continue
            Color.RED -> if (redCards<maxRedCards) redCards++ else continue
            Color.BLUE -> if (blueCards<maxBlueCards) blueCards++ else continue
            Color.WHITE -> if (whiteCards<maxWhiteCards) whiteCards++ else continue
        }
        cards[totalCards] = Card(randomColor, wordList[(wordList.indices).random()])
        for (i in 0..cards.size-2){
            if (cards[totalCards]?.text == cards[i]?.text) {
                continue
            }
        }
        totalCards++
